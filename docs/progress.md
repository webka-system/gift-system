...
### 0.16 追補（2026-07-09）：受け取り者ページのUI/UX整え（見た目・使い勝手のみ・機能不変）
初めて使う一般の人がスマホで迷わず入力を終えられることを最優先に、見た目と使い勝手を整えた。機能・ロジック
（トークン照合・二重確定防止・バリデーション・確定処理・商品詳細モーダル・期限切れ/使用済み表示）は完全維持。
店舗ヘッダー/フッター（濃紺）と調和。

- **必須/任意の明示**（最優先）：住所入力フォームの各項目に **`必須`（赤）/`任意`（灰）チップ**を追加。上部に
  「`必須` は入力が必要な項目です。」の凡例。配達希望日・時間帯は `任意` を明示し「未入力のまま進めていただけます」を追記。
- **入力エラーの分かりやすさ**：確定時に未入力/不正があると、**該当欄を赤枠でハイライト**し最初の欄へフォーカス＆
  スクロール、エラーは目立つ注意ボックス（⚠アイコン）に。メッセージも「未入力の必須項目があります。赤枠の欄を…」に。
  入力を直すと赤枠は即解除。**検証ルール自体は不変**（表示のみ追加。receive.js の fail() に対象フィールドを渡すだけ）。
  商品未選択時は商品一覧を赤枠で強調してスクロール。
- **ステップの可視化**：「1 商品を選ぶ / 2 お届け先を入力 / 3 配達希望（任意）」を**番号バッジ付きの見出し**にして、
  今どの段階か・次に何をするかを分かりやすく。
- **スマホの打ちやすさ**：入力欄の **font-size を 16px** に（iOS のフォーカス時自動ズームを防止）、パディング・項目間隔を拡大、
  ラベルのキャプションを太字化して入力欄との対応を明確に。
- **主要ボタンの押しやすさ**：「確定する」「この商品を選ぶ」を大きめ・太字・影付きに、押下時の沈み込みフィードバック。
- **商品選択の分かりやすさ**：選択中カードに「**選択中**」ラベル＋内側リング、「詳細を見る」をタップしやすいサイズ（›付き）。
- **郵便番号→住所補完のフィードバック**：`#zip-status` を状態で色分け＋アイコン（🔍検索中／✓自動入力しました／⚠見つからない・失敗）。
  補完で都道府県・住所が入ったら該当欄の赤枠も解除。
- **デザイントークン**：`--radius`/`--shadow`/`--danger-weak` 等を追加し角丸・影・色を統一（管理画面の方向性と揃えつつ、
  受け取り者ページはより親しみやすく）。
- **変更ファイル**：`web/receive/index.html`（チップ・凡例・ステップ見出しの表示テキスト／構造・name・id は不変）／
  `web/receive/css/receive.css`（見た目の調整）／`web/receive/js/receive.js`（**表示のみ**：フィールド赤枠ハイライト・
  zip状態クラス・入力で赤枠解除。検証条件・確定処理・fetch は不変）。
- **検証**：hosting エミュレータで配信を確認（必須チップ8＝7項目＋凡例1／任意3／ステップ3／16px入力・invalid・
  zip状態・選択中バッジ・エラーボックス・ボタンactive／JSのmark/clearInvalid・赤枠強調／全アセット200）。
  web 構文チェック緑・CSS波括弧一致。機能に触れていないためロジック検証は不要。
- **デプロイ**：**hosting のみ＝`firebase deploy --only hosting`**（受け取り者ページのみ）。functions/新規HTTP関数なし＝
  Cloud Run手動public設定は不要。

### 0.15 追補（2026-07-09）：管理画面のUI/UX整え（見た目・使い勝手のみ・機能不変）
機能・ロジック・id・クラス名は一切変更せず、CSS中心＋HTMLは表示テキスト（必須/任意チップ）のみで見やすく整えた。
QR一覧・ステータス色分け・詳細モーダル基盤・受け取り者ヘッダー等の既存調整と調和させ、業務ツールとして「探しやすい・
迷わない・視認性が高い」ことを基準に、過度な装飾は避けた。

- **デザイントークン**：`--radius`/`--shadow-sm`/`--accent-weak`/`--row-hover`/`--row-zebra` 等を追加し、角丸・影・
  余白・色を統一。背景をわずかに落として（#f4f5f7）カードが浮くように。
- **タイポグラフィ/セクション**：h2 に薄い区切り線＋余白で「まとまりの начало」を明確化。行間 1.55。
- **タブ**：ホバー背景＋アクティブをアクセント色文字に。今どこにいるか分かりやすく。
- **テーブル**：淡い影＋**ゼブラ（偶数行）＋行ホバー**で長い一覧を目で追いやすく。ヘッダー行を小さめ・字間で軽く。
- **フォーム**：カード化（影・余白）＋ラベルをミュート太字で値と区別。**必須/任意をチップで明示**（req-chip=赤/opt-chip=灰。
  種別・商品・QR生成フォームの各項目）。入力欄ホバーで枠を強調。
- **絞り込みバー（.filters）**：薄いカードでひとまとめにして項目を探しやすく（印刷タブの操作カードと統一感）。
- **ボタン階層**：主要操作（保存・生成・出力）＝塗り（太字）、副次（行内・キャンセル）＝白ベース、
  **危険操作（削除）＝赤字＋赤枠ホバー**で注意喚起（`[data-act="delete"]` を見た目だけ強調・ロジック不変）。
  フォーカスリング（focus-visible）も追加。
- **詳細モーダル**：ラベル/値の色・余白・行間を調整して読みやすく。閉じるボタンを控えめな白ボタンに統一。
  小画面ではラベルと値を縦積み・余白最適化。
- **レスポンシブ**：640px 以下で form-grid 1カラム・余白圧縮・モーダル余白調整（10インチ〜スマホで崩れない）。
- **変更ファイル**：`web/admin/css/admin.css`（全面的な見た目の調整）／`web/admin/index.html`（必須/任意チップの
  表示テキストのみ。id・input・ボタンの構造は不変）。**JS不変**（クラス/idは既存のまま）。
- **検証**：CSS 波括弧バランス一致・主要新スタイル（ゼブラ/行ホバー/req-chip/削除赤/タブホバー/影）の配信・
  必須チップ6/任意チップ6・全アセット200 を hosting エミュレータで確認。機能に触れていないためロジック検証は不要。
- **デプロイ**：**hosting のみ＝`firebase deploy --only hosting`**（管理画面の CSS/HTML のみ）。functions/新規HTTP関数
  なし＝Cloud Run手動public設定は不要。**元に戻したい箇所があれば個別に戻せる**（トークン・各ブロック単位の変更）。

### 0.14 追補（2026-07-09）：受け取り者ページに店舗ヘッダー/フッターを追加（信頼性担保）
受け取り者は「知らないURL」を開くため、店舗の識別・信頼性担保のため全画面共通のヘッダー/フッターを追加。
- **ヘッダー**：店舗ロゴ（`header-logo_sp.svg`・紺#1b2f69/金#b09859）を白背景で表示。`<main>` の外に置き全画面共通。
- **フッター**：店舗ロゴ（`footer-logo_sp.svg`・**白版**）＋コピーライト「© KYUSHU OTORIYOSE HONPO All Rights Reserved.」を
  **濃紺 #081746** 背景で表示。フッターロゴは白版なので濃紺で視認OK（配色調整不要）。`<main>` の外＝全画面共通。
- **全画面共通の仕組み**：ヘッダー/フッターは `.view`（商品選択・詳細モーダル・住所入力・確定完了・期限切れ・使用済み・
  無効/エラー）の外側に配置。`show()` は `.view` の hidden だけを切り替えるため、どの画面でもヘッダー/フッターは常時表示。
- **レイアウト**：body を flex 縦積み（`min-height:100vh` / `.app-shell { flex:1 0 auto }`）でスティッキーフッター化
  （内容が短くてもフッターは最下部）。ロゴは横長(約11.5:1)を幅制御（`max-width:300px`）でスマホに自然に収める。
  ノッチ端末向けにフッター下に `env(safe-area-inset-bottom)`。
- **ロゴ**：外部URL（file.otoriyose.site）を img src で直参照（両方 200・image/svg+xml で到達確認）。管理画面は対象外。
- **将来拡張**：フッターに法的表記リンク（プライバシーポリシー・特商法）を後から足せるよう `.site-legal` のコメント枠を用意。
- **変更ファイル**：`web/receive/index.html`（header/footer 追加・modal を main 外へ移動）／`web/receive/css/receive.css`
  （スティッキーフッター・ヘッダー/フッター/ロゴ/コピーライトのスタイル）。receive.js は不変（modal は id 参照）。
- **検証**：hosting エミュレータで配信DOM（header→main→footer の順・ヘッダー/フッターが .view の外＝全画面共通・
  ロゴURL・コピーライト・法的枠コメント）とCSS（濃紺#081746・ロゴ幅・スティッキー）と外部ロゴ到達(200)を確認。web構文不変。
- **デプロイ**：**hosting のみ＝`firebase deploy --only hosting`**（受け取り者ページのみ）。functions/新規HTTP関数なし＝
  Cloud Run手動public設定は不要。

### 0.13 追補（2026-07-09）：店舗2にギフト用パターン11（フォーマット90/汎用）を新規取得 → これを使う
0.11 で記録した「パターン4は makeshop実受注用フォーマット(100035)で、汎用マッピングと噛み合わなければ店舗2に汎用
フォーマット(90)のパターンを新規作成する」という対処案が**実現**した。店舗2に gift 用パターンを新規取得済み。

- **新規取得したパターン**：パターンID:**11** / 店舗ID:2 / 名前「九州お取り寄せ本舗(makeshop扱い ギフトカード)」/
  フォーマットID:**90（汎用標準）** / 状態:**無効**。
- **店舗2には現在2パターン**：4（makeshop実受注用・100035・gift用でない）と 11（gift用・90汎用）。
  **gift-system は 11 を使う**（フォーマット90＝汎用なので 0.12 で完全一致させたCSVマッピングと整合）。
- **反映（コード）**：
  - `NE_KNOWN_PATTERN_ID_SHOP2` を "4"→**"11"**（検算値）。`NE_GIFT_PATTERN_NAME_HINT="ギフトカード"` を追加。
  - `resolveUploadPatternId(shopId, deps, opts?)` に **opts.nameContains** を追加（店舗2に複数パターンがある場合に
    名前で gift 用を選別）。実接続では `resolveUploadPatternId("2", deps, { nameContains:"ギフトカード" })` → 11 を得る。
  - `config/env.ts` の `NE_UPLOAD_PATTERN_ID` コメントを想定値=11・フォーマット90・無効→有効化必要 に更新。
- **★実接続前に必須**：パターン11は現在 deleted_flag=**無効**。**NE管理画面で店舗2のパターン11（ギフトカード）を
  有効化**すること（無効/存在しないIDでアップロードすると「存在しない受注一括登録パターン」エラーで弾かれる）。
  有効化前は resolveUploadPatternId が null を返す＝有効化が必要のサイン。
- **フォーマット懸念の解消**：11は汎用90なので gift-system の汎用前提マッピング（0.12の41列CSV）と整合する想定。
  実 CSV取込で区分変換警告等が出ないかは実接続時に最終確認。
- **検証**：疑似 info レスポンスで単体確認 5/5（既知値=11／名前「ギフトカード」で絞ると11／名前絞り無しは先頭4で曖昧
  ＝名前絞りが必要／giftが無効なら null）。functions lint/build 緑。
- **デプロイ**：functions のみ（記録/スタブで runtime 不変・dormant）。新規HTTP関数なし＝Cloud Run手動public不要。

### 0.12 追補（2026-07-09）：NE取込CSVを汎用パターンの正解仕様（41列）に完全一致
NE取り込みで「受注住所１が存在しません」等のエラーが出た原因＝CSVの列名・構造がNE汎用パターンの期待と不一致。
サンプルCSVから正解仕様が判明したので `ne/csv.ts`（NE_CSV_COLUMNS）と `admin-ne.ts`（行生成）を**完全一致**に修正。

- **41列・順序・ヘッダー名を一字一句一致**（全角「受注住所１」「消費税率（%）」等も正確に）。文字コードは Shift-JIS 維持。
- **列マッピング**（gift-system → NE）:
  - 店舗伝票番号=token（buildSlipNo）／受注日=usedAt を **yyyy/MM/dd HH:mm:ss**(JST)／郵便番号・電話=**数字のみ**
  - **受注住所１=都道府県+address（空にしない＝エラー解消）／受注住所２=building**（gift-systemは address 1フィールドのため
    住所1に都道府県+address、住所2に building を入れる分け方）
  - 受注ブロック＝発送先ブロック（受け取り者情報を複製）／メール=recipientEmail
  - 支払方法「ポイント全額払い」／発送方法「宅急便」(仮)／金額系(商品計・税金・発送料・手数料・ポイント・その他・
    合計)=0・ギフトフラグ=0・商品価格=0・受注数量=1
  - **時間帯指定=「時間帯指定[○○]」形式**（NE_DELIVERY_TIME_MAP：午前中/14時-16時/16時-18時/18時-20時/19時-21時、
    未指定は空）／**日付指定=yyyy/MM/dd**（未指定は空）／備考=memo／その他の空欄列は ""
- **変更ファイル**：`functions/src/ne/csv.ts`（NeCsvRow/NE_CSV_COLUMNS を41列へ）／`functions/src/ne/order.ts`
  （NE_DELIVERY_TIME_MAP を「○時-○時」表記へ）／`functions/src/http/admin-ne.ts`（住所1/2分割・数字のみ・
  日時/日付整形・不要なtype解決/joinAddress除去）／`shared/constants.js`（発送方法「宅急便」仮置き）／テスト更新。
- **検証**：ユニット（buildNeCsvString の41列ヘッダー厳密一致・住所1非空・時間帯[○○]・38 passing）＋
  エミュレータ end-to-end で **実CSVを Shift-JIS デコードして 28/28**（ヘッダー完全一致・受注住所１非空・
  受注日/郵便/電話/時間帯/日付/金額の形式・発送先複製・備考=memo・末尾空欄）。functions lint/build 緑。
- **実接続時に私が確認する残項目**：支払方法「ポイント全額払い」がNE区分に存在するか／発送方法の正確な表記／
  時間帯指定のNEが受け付ける正確な表記（NE_DELIVERY_TIME_MAP の右辺）。
- **デプロイ**：**functions のみ＝`firebase deploy --only functions`**（CSV生成はサーバ側。hosting導線は不変）。
  新規HTTP関数なし＝Cloud Run手動public設定は不要。

### 0.11 追補（2026-07-09）：NE店舗2パターンIDの実測値と実接続時の要確認（GAS実測反映）
既存のNE連携済GASで受注一括登録パターン情報取得APIを実際に叩いて判明した確定情報を反映・記録（実送信は審査後）。

**GAS実測で判明した確定値**
- **店舗ID:2（makeshop）の受注一括登録パターンID = 4**。パターン名「九州お取り寄せ本舗(makeshop)(ネクストエンジンカート)」、
  フォーマットID:100035。
- gift-system は店舗2に **パターンID:4** で投入する（月次集計の都合で店舗2確定。楽天/Yahoo/amazon/LINEギフト等は無関係）。
- 前回実装の `resolveUploadPatternId("2")` が返すべき値は **4**（GAS実測と一致＝ロジックは正しい）。

**反映（コード）**
- `ne/upload-pattern.ts`：既知値 `NE_KNOWN_PATTERN_ID_SHOP2 = "4"`（検算/フォールバック用）を定義。
  `resolveUploadPatternId` に **有効フラグ絞り込みを追加**（`receive_order_upload_pattern_deleted_flag` を取得し、
  `"1"（無効）を除外`＝有効パターンだけを照合）。該当店舗が無効パターンのみのとき `hasInactiveOnly:true` をログに出す。
- `config/env.ts`：`NE_UPLOAD_PATTERN_ID` の想定値が **4** である旨と、無効フラグの注意をコメント明記。
  （既定は空のまま＝dormant。resolveUploadPatternId による自動特定を基本とし、4 は検算に使う。）

**★実接続時の要確認①（無効パターン）**
- GAS実測時点で店舗2のパターン4は **deleted_flag が「無効」**（会社は普段API自動連携で取込しており、手動アップロード用
  パターンを使っていないため）。NE仕様上、**無効/存在しないパターンIDをアップロードAPIに渡すと「存在しない受注一括登録
  パターン」エラーで弾かれる可能性**がある。
- 対処：実接続でアップロードが弾かれたら、**NE管理画面で店舗2の受注一括登録パターンを有効化**する。
  `resolveUploadPatternId` は有効パターンだけを返す（無効のみなら null）ので、null が返る＝有効化が必要のサイン。

**★実接続時の要確認②（フォーマット適合）**
- パターン4のフォーマットは「ネクストエンジンカート形式(100035)」＝makeshop実受注用の形式。
  gift-system のマッピング（汎用フォーマット前提で構築した ne/order.ts・csv.ts）がこの **100035形式と噛み合うかは実接続時に
  要確認**。
- 合わない場合の対処案：**店舗2に汎用フォーマット(90)の受注一括登録パターンを新規作成し、そのIDを使う**（店舗2のまま、
  フォーマットだけ汎用にする）。この場合 `NE_UPLOAD_PATTERN_ID` は新規作成した汎用パターンのIDになる（4ではなくなる）。

**検証**：`resolveUploadPatternId` を疑似 info レスポンスで単体確認（既知値=4／店舗2の有効パターン→4／無効のみ→null／
有効・無効混在→有効IDを返す：4/4）。functions lint/build/test(35 passing) 緑。
**デプロイ**：functions のみ（実接続前の記録/スタブで runtime 不変・dormant）。新規HTTP関数なし＝Cloud Run手動public不要。
審査後のNE実装とまとめてデプロイでも可。
